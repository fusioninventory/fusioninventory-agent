...
sub initModList {
    ...
    if ( !$@ ) {
        $logger->debug(
            "use Ocsinventory::Agent::Backend::ModuleToLoad to get the modules "
    ...
    if ( $config->{devlib} ) {
        ...
        eval { @installed_files = $inst->files('Ocsinventory') };
    ...
    if (@dirToScan) {
        ...
        if ($@) {
            ...
            File::Find::find(
                {
                    wanted => sub {
                        push @installed_files, $File::Find::name
                          if $File::Find::name =~
            ...
            @{ ${ $self->{modules}->{$m}->{runMeIfTheseChecksFailed} } } )
        {
            if ( $self->{modules}->{$condmod}->{inventoryFuncEnable} ) {
                foreach ( keys %{ $self->{modules} } ) {
                    next
                      unless /^$m($|::)/
                          && $self->{modules}->{$_}->{inventoryFuncEnable};
                    $self->{modules}->{$_}->{inventoryFuncEnable} = 0;
                    $logger->debug(
"$_ disabled because of a 'runMeIfTheseChecksFailed' in '$m'\n"
                    );
                }
            }
        }
...
sub retrieveStorage {
    ...
    if ( !exists &retrieve ) {
        ...
        if ($@) {
            $logger->debug(
                "Storable.pm is not avalaible, can't load Backend module data");
            return;
        }
    }
...
sub saveStorage {
    ...
    if ( !exists &store ) {
        ...
        if ($@) {
            $logger->debug(
                "Storable.pm is not avalaible, can't save Backend module data");
            return;
        }
    }
...
sub runRpc {
    ...
    if ($@) {
        $logger->info(
            "HTTP::Server::Brick is not installed, rpc mode disabled");
        return;
    }
    ...
    foreach my $m ( sort keys %{ $self->{modules} } ) {
        ...
        foreach ( keys %$cfg ) {
            ...
            if ( exists( $cfg->{$_}->{path} ) ) {
                ...
                push @HTTPServerBrickMountPoints,
