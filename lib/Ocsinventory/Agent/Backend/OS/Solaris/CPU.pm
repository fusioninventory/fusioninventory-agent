...
#
  ...
  # a generic methode
    ...
    if ( $sun_class_cpu == 1 ) {
        foreach (`memconf 2>&1`) {
            if (
/^Sun Microsystems, Inc. Sun Fire\s+\S+\s+\((\d+)\s+X\s+(\S+)\s+(\d+)/
              )
            {
                $cpu_slot  = $1;
                $cpu_type  = $2;
    ...
    if ( $sun_class_cpu == 2 ) {
        foreach (`memconf 2>&1`) {
            if (
/^Sun Microsystems, Inc. Sun Fire\s+\S+\s+\((\d+)\s+X\s+(\S+)\s+(\S+)\s+(\d+)/
              )
            {
                $cpu_slot  = $1;
                $cpu_type  = $3 . " (" . $2 . ")";
    ...
    if ( $sun_class_cpu == 3 ) {
        foreach (`memconf 2>&1`) {
            if (
/^Sun Microsystems, Inc.\s+\S+\s+\(\S+\s+\S+\s+\S+\)\s+\((\S+)\s+(\S+)\s+(\S+)\s+(\d+)/
              )
            {
    ...
    if ( $sun_class_cpu == 4 ) {
        foreach (`memconf 2>&1`) {
            if (
/^Sun Microsystems, Inc\..+\((\S+)\s+(\S+)\s+(\S+)\s+(\d+)(\w+)\)$/
              )
            {
                $cpu_slot  = 1;
                $cpu_type  = $3 . " (" . $1 . " " . $2 . ")";
