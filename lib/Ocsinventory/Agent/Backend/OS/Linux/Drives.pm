...
s{^\s+(\S+) = (.*) \s*\((int|string|bool|string list|uint64)\)}{}
    ...
    foreach (`df -TP`) {    # TODO retrive error
        ...
        if (/^(\S+)\s+(\S+)\s+(\S+)\s+(?:\S+)\s+(\S+)\s+(?:\S+)\s+(\S+)\n/) {
            ...
            # no virtual FS
            next
              if ( $filesystem =~ /^(tmpfs|usbfs|proc|devpts|devshm|udev)$/ );
            next if ( $type =~ /^(tmpfs)$/ );
...
/Filesystem created:\s+\w+ (\w+) (\d+) ([\d:]+) (\d{4})$/
                      )
                    {
                        $createdate =
                          $4 . '/' . $months{$1} . '/' . $2 . ' ' . $3;
            ...
            if ( keys %listVolume ) {
                if ( defined $listVolume{$volumn} ) {
                    if ( $filesystem eq '' ) {
                        $filesystem = $listVolume{$volumn}->{'volume.fstype'};
                    }
                    if ( $label eq '' ) {
                        $label = $listVolume{$volumn}->{'volume.label'};
                    }
                    if ( $total eq '' ) {
                        $total =
                          int( $listVolume{$volumn}->{'volume.size'} /
                              ( 1024 * 1024 ) + 0.5 );
                    }
                    if ( $type eq '' ) {
                        $type = $listVolume{$volumn}->{'storage.model'};
                    }
                    if ( $serial eq '' ) {
                        $serial = $listVolume{$volumn}->{'volume.uuid'};
                    }
