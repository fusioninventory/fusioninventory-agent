...
sub doInventory {
    ...
    if ( defined( $gateway{'0.0.0.0'} ) ) {
        $inventory->setHardware( { DEFAULTGATEWAY => $gateway{'0.0.0.0'} } );
    }
...
# replace '0.0.0.0' (ie 'default gateway') by the default gateway IP adress if it exists
            if (    defined($ipgateway)
                and $ipgateway eq '0.0.0.0'
                and defined( $gateway{'0.0.0.0'} ) )
            {
            ...
            if ( -d "/sys/devices/virtual/net/" ) {
                $virtualdev =
                  ( -d "/sys/devices/virtual/net/$description" ) ? "yes" : "no";
            }
            else {
        ...
        if ( $line =~ /^$/ ) {    # End of section
            ...
            $description = $driver = $ipaddress = $ipgateway  = $macaddr =
              $pcislot   = $status = $type      = $virtualdev = undef;
