...
sub getFromUdev {
    ...
    foreach ( glob("/dev/.udev/db/*") ) {
        my (
            $scsi_coid, $scsi_chid, $scsi_unid,    $scsi_lun,
            $path,      $device,    $vendor,       $model,
            $revision,  $serial,    $serial_short, $type,
            $bus,       $capacity
        );
...
sub getCapacity {
    my ($dev) = @_;
    my $cap;
    chomp( $cap = `fdisk -s /dev/$dev 2>/dev/null` )
...
sub getManufacturer {
    my ($model) = @_;
    if ( $model =~
/(maxtor|western|sony|compaq|hewlett packard|ibm|seagate|toshiba|fujitsu|lg|samsung|nec|transcend)/i
      )
    {
        return ucfirst( lc($1) );
    }
    ...
    if ( can_run("lshal") ) {
        ...
        foreach my $line (`lshal`) {
            ...
            if ( $line =~ s{^udi = '/org/freedesktop/Hal/devices/storage.*}{} )
            {
                ...
                and $line =~
s{^\s+(\S+) = (.*) \s*\((int|string|bool|string list|uint64)\)}{}
              )
            {
                my $key   = $1;
                my $value = $2;
...
#Get hard drives values from sys or proc in case getting them throught udev doesn't work
    if ( !%$devices ) {
        my ( $manufacturer, $model, $media, $firmware, $serialnumber, $capacity,
        ...
        foreach my $device ( keys %$partitions ) {
            ...
            if ( !$devices->{$device}->{MANUFACTURER} ) {
                $devices->{$device}->{MANUFACTURER} =
                  getFromSysProc( $device, "vendor" );
            }
            if ( !$devices->{$device}->{MODEL} ) {
                $devices->{$device}->{MODEL} =
                  getFromSysProc( $device, "model" );
            }
            if ( !$devices->{$device}->{TYPE} ) {
                $devices->{$device}->{TYPE} =
                  getFromSysProc( $device, "removable" ) ? "removable" : "disk";
            }
            if ( !$devices->{$device}->{FIRMWARE} ) {
                $devices->{$device}->{FIRMWARE} =
                  getFromSysProc( $device, "rev" );
            }
            if ( !$devices->{$device}->{SERIALNUMBER} ) {
                $devices->{$device}->{SERIALNUMBER} =
                  getFromSysProc( $device, "serial" );
            }
...
#    if (($devices->{$device}->{MANUFACTURER} ne 'AMCC') and ($devices->{$device}->{MANUFACTURER} ne '3ware') and ($devices->{$device}->{MODEL} ne '') and ($devices->{$device}->{MANUFACTURER} ne 'LSILOGIC') and ($devices->{$device}->{MANUFACTURER} ne 'Adaptec')) {
        ...
        if (  !$devices->{$device}->{MANUFACTURER}
            or $devices->{$device}->{MANUFACTURER} eq 'ATA' )
        {
            $devices->{$device}->{MANUFACTURER} =
              getManufacturer( $devices->{$device}->{MODEL} );
        }

        if ( $devices->{$device}->{CAPACITY} =~ /^cdrom$/ ) {
            $devices->{$device}->{CAPACITY} =
              getCapacity( $devices->{$device}->{NAME} );
        }
