...
sub doInventory {
    ...
    my (
        $card,     $card_model,   $unit,        $unit_id,
        $port,     $serialnumber, $serial,      $model,
        $capacity, $firmware,     $description, $media,
        $device,   $manufacturer, $sn
    );
 ...
 # Allow us to associate a node to a drive : sda -> WD-WMANS1648590
                    ...
                    foreach (`tw_cli info $card $unit`) {
                        ...
                        if ($port) {
                            ...
                            foreach (
`tw_cli info $card $port model serial capacity firmware`
                              )
                            {
...
# Need to be tested on a system with multiple 3ware cards.
                                if (
                                    ( $hd->{SERIALNUMBER} eq 'AMCC_' . $sn )
                                    or (  $hd->{MODEL} eq 'Logical_Disk_'
                                        . $unit_id )
                                  )
                                {
                                    $device = %$hd->{NAME};
                ...
                # SATA drives only to 7xxx/8xxx/9xxxx cards
                            ...
                            $port = undef;
                            $logger->debug(
"3ware: $device, $manufacturer, $model, $description, $media, $capacity, $serialnumber, $firmware"
                            );
