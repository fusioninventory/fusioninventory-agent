...
sub doInventory {
    ...
    if ( -r '/proc/scsi/scsi' ) {
        foreach my $hd (@devices) {
            ...
            my ( $host, $model, $firmware, $manufacturer, $size,
                $serialnumber );
            my $count = -1;
            while (<PATH>) {
                ( $host, $count ) = ( 1, $count + 1 )
                  if /^Host:\sscsi$hd->{SCSI_COID}.*/;
                if ($host) {
                        ...
                        and ( $1 !~ 'raid.*' ) )
                    {
                        ...
                        $logger->debug(
"Adaptec: $hd->{NAME}, $manufacturer, $model, SATA, disk, $hd->{DISKSIZE}, $serialnumber, $firmware"
                        );
                        $host = undef;
