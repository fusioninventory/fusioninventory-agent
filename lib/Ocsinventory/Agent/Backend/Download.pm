...
sub check {
    ...
    foreach (@$conf) {
        if ($_->{TYPE} eq 'CONF') {
            ...
            next unless $_->{ID} =~ /^\d+$/; # Security
            my $targetDir = $downloadBaseDir.'/'.$_->{ID};
            if (!-d $targetDir && !mkdir ($targetDir)) {
                $logger->error("Failed to create $targetDir");
            }
            my $infoURI = 'https://'.$_->{INFO_LOC}.'/'.$_->{ID}.'/info';
            if (!is_success(getstore($infoURI, $targetDir.'/info'))) {
                $logger->error("Failed order info file from $infoURI to ".$targetDir.'/info');
            }
