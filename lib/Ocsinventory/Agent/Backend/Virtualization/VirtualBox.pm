...
sub isInventoryEnabled {
    return can_run('VirtualBox')
      and can_run('VBoxManage');
}
...
sub doInventory {
    ...
    {    # read only the information on the first paragraph of each vm
        ...
        if ( $in == 0 and $line =~ m/^Name:\s+(.*)$/ ) {    # begin
            ...
            if ( $line =~ m/^\s*$/ ) {                      # finish
                ...
                $name = $status = $mem = $uuid =
                  "N\A";   # useless but need it for security (new version, ...)
        ...
        if ( $line !~ m/^root/ ) {
            if ( $line =~ m/^.*VirtualBox (.*)$/ ) {
                ...
                if ( $scanhomedirs == 1 ) {   # If I will scan Home directories,
                    $vmRunnings[$index] =
                      $uuid;                  # save the no-root running machine
                    $index += 1;
                }
                else {
                    $inventory->addVirtualMachine(
    ...
    if ( $scanhomedirs == 1 ) {
        ...
        foreach
          my $xmlMachine ( bsd_glob("/home/*/.VirtualBox/Machines/*/*.xml") )
        {
            chomp($xmlMachine);
                ...
                $inventory->addVirtualMachine(
                    {
                        ...
                        UUID => $uuid,
                        MEMORY =>
                          $data->{Machine}->{Hardware}->{Memory}->{RAMSize},
                        STATUS    => $status,
                        SUBSYSTEM => "Sun xVM VirtualBox",
                        VMTYPE    => "VirtualBox",
                    }
        ...
        foreach
          my $xmlVirtualBox ( bsd_glob("/home/*/.VirtualBox/VirtualBox.xml") )
        {
            chomp($xmlVirtualBox);
            ...
            if (    $defaultMachineFolder != 0
                and $defaultMachineFolder != "Machines"
                and $defaultMachineFolder =~
                /^\/home\/S+\/.VirtualBox\/Machines$/ )
            {

                foreach my $xmlMachine (
                    bsd_glob( $defaultMachineFolder . "/*/*.xml" ) )
                {
                    my $configFile = new XML::Simple;
                    my $data       = $configFile->XMLin($xmlVirtualBox);

                    if (    $data->{Machine} != 0
                        and $data->{Machine}->{uuid} != 0 )
                    {
                        my $uuid = $data->{Machine}->{uuid};
                        $uuid =~ s/^{?(.{36})}?$/\1/;
                        my $status = "off";
                        ...
                        $inventory->addVirtualMachine(
                            {
                                NAME => $data->{Machine}->{name},
                                VCPU =>
                                  $data->{Machine}->{Hardware}->{CPU}->{count},
                                UUID   => $uuid,
                                MEMORY => $data->{Machine}->{Hardware}->{Memory}
                                  ->{RAMSize},
                                STATUS    => $status,
                                SUBSYSTEM => "Sun xVM VirtualBox",
                                VMTYPE    => "VirtualBox",
