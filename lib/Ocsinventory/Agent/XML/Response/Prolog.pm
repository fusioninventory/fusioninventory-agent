...
sub updatePrologFreq {
    ...
    if ( $parsedContent && exists( $parsedContent->{PROLOG_FREQ} ) ) {
        if ( $parsedContent->{PROLOG_FREQ} ne
            $self->{accountconfig}->get("PROLOG_FREQ") )
        {
            $logger->info( "PROLOG_FREQ has changed since last process(old="
                  . $self->{accountconfig}->get("PROLOG_FREQ") . ",new="
                  . $parsedContent->{PROLOG_FREQ}
                  . ")" );
            $self->{prologFreqChanged} = 1;
            $self->{accountconfig}
              ->set( "PROLOG_FREQ", $parsedContent->{PROLOG_FREQ} );
        }
        else {
...
sub saveNextTime {
    ...
    if ( !open NEXT_TIME, ">" . $self->{config}->{next_timefile} ) {
        $logger->error( "Cannot create the next_timefile `"
              . $self->{config}->{next_timefile}
              . "': $!" );
        return;
    }
    ...
    if ( $self->{prologFreqChanged} ) {
        ...
        $time =
          time +
          int
          rand( ( $serverdelay ? $serverdelay : $self->{config}->{delaytime} ) *
              3600 );
    }
    else {
        $time = time +
          ( $serverdelay ? $serverdelay : $self->{config}->{delaytime} ) * 3600;
    }
