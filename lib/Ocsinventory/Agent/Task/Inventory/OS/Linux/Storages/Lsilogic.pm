package Ocsinventory::Agent::Task::Inventory::OS::Linux::Storages::Lsilogic;

use Ocsinventory::Agent::Task::Inventory::OS::Linux::Storages;
...
sub doInventory {
  ...
  my @devices = Ocsinventory::Agent::Task::Inventory::OS::Linux::Storages::getFromUdev();
...
#scsi_id:0 100%
      ...
      if (/.*phys_id:(\d+).*product_id:\s*(\S*)\s+revision:(\S+).*size\(GB\):(\d+).*/) {
        ...
        my $firmware = $3;
        my $manufacturer = Ocsinventory::Agent::Task::Inventory::OS::Linux::Storages::getManufacturer($model);
