package Ocsinventory::Agent::Task::Inventory::OS::Linux::Storages::Adaptec;
      ...
      while (<PATH>) {
        ...
        if ($host) {
          if ((/.*Model:\s(\S+).*Rev:\s(\S+).*/) and ($1 !~ 'raid.*')) {
            $model = $1;
            $firmware = $2;
            $manufacturer = Ocsinventory::Agent::Task::Inventory::OS::Linux::Storages::getManufacturer($model);
