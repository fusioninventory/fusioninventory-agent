...
sub _handle_now {
    ...
    BLOCK: {
        foreach my $controller ($self->{agent}->getControllers()) {
            next unless $controller->isa('FusionInventory::Agent::Controller::Server');
            my $url       = $controller->getUrl();
            ...
            $code    = 200;
            $message = "OK";
            $trace   = "rescheduling next contact for controller $url right now";
            last BLOCK;
        }
        ...
        if ($self->_isTrusted($clientIp)) {
            ...
            $code    = 200;
            $message = "OK";
            $trace   = "rescheduling next contact for all controllers right now";
            last BLOCK;
        }
