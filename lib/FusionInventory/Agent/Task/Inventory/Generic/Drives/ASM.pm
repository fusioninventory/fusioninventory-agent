...
sub doInventory {
    ...
    my $grid_home = $user eq 'root' ? $ENV{GRID_HOME} : getFirstLine(command => "su - $user -c 'echo \$GRID_HOME'");
    ...
    my $cmd = ($grid_home ? "ORACLE_SID=$asm ORACLE_HOME=$grid_home $grid_home/bin/" : "")."asmcmd lsdg";
