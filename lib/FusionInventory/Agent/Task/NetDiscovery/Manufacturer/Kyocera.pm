...
sub discovery {
    ...
    if ($description =~ m/,HP,JETDIRECT,J/) {
        my $description_new = $snmp->get('.1.3.6.1.4.1.1229.2.2.2.1.15.1');
        ...
        my $description_new = $snmp->get('.1.3.6.1.4.1.1347.42.5.1.1.2.1');
        if (($description_new ne "null") && ($description_new ne "No response from remote host")) {
            $description = $description_new;
        } elsif ($description_new eq "No response from remote host") {
            my $description_new = $snmp->get('.1.3.6.1.4.1.1347.43.5.1.1.1.1');
            if (($description_new ne "null") && ($description_new ne "No response from remote host")) {
                $description = $description_new;
            } elsif ($description_new eq "No response from remote host") {
                my $description_new = $snmp->get('.1.3.6.1.4.1.1347.43.5.1.1.1.1');
